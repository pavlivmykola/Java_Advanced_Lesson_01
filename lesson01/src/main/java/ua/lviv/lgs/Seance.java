...
public class Seance implements Comparable<Seance>{
	...
	private void validTime(Time startTime, Time endTime, Time timeOpen, Time timeClose) throws TimeException {
		if (endTime.getHour()>=timeClose.getHour())  {
			throw new TimeException("Надто пізня година");
		}
		if  (endTime.getHour()==timeClose.getHour() && endTime.getMin()>timeClose.getMin()) {
			throw new TimeException("Надто пізня година");
		}
		if (startTime.getHour()<=timeOpen.getHour()) {
			throw new TimeException("Надто рання година");
		}
